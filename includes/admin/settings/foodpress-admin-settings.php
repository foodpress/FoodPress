...
if ( ! function_exists( 'foodpress_settings' ) ) {
	...
	function foodpress_settings() {
		...
		if( isset($_POST['food_noncename']) && isset( $_POST ) ){
			if ( wp_verify_nonce( $_POST['food_noncename'], 'food_nonce' ) ){
...
<div class="wrap" id='fp_settings'>
	<h2>FoodPress Settings <small class="foodpress_note" style="font-size:60%;">v<?php echo get_option('foodpress_plugin_version'); ?></small></h2>
	<?php do_action('foodpress_updates_in_settings');?>
	<h2 class='nav-tab-wrapper' id='meta_tabs'>
		<?php
			foreach($food_tabs as $nt=>$ntv){
				$food_notification='';
				if($nt == 'food_ie') {
					echo "<a href='?page=food_ie' class='nav-tab ".( ($focus_tab == $nt)? 'nav-tab-active':null)."' food_meta='food_1'>".$ntv.$food_notification."</a>";
				} else {
...
<?php
	...
	// Reservations TAB
		case "food_6":
			...
			$reservation = new fp_settings_reservation($food_opt);
			$reservation->get_content();
